...
struct SFMLRenderer::SFMLRendererImpl {
  ...
  SFMLRendererImpl(unsigned width, unsigned height, const string windowName)
      : window(sf::VideoMode(width, height), sf::String()),
  ...
  void DrawRectangle(const Vector2 &halfExtents, const Vector2 &pos) {
    sf::RectangleShape rect(
        sf::Vector2f(halfExtents.x * 2.0f, halfExtents.y * 2.0f));
...
SFMLRenderer::SFMLRenderer(unsigned width, unsigned height,
...
void SFMLRenderer::DrawCircle(const Vector2 &pos, float radius) {
  impl->DrawCircle(pos, radius);
}

void SFMLRenderer::DrawRectangle(const Vector2 &halfExtents,
                                 const Vector2 &pos) {
